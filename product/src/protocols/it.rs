...
pub fn bgp(b: &[u8]) -> Result<Decoded> {
    ...
    if !(19..=4096).contains(&n) {
        return Err(bad(
            "bgp_length",
            "length outside base BGP range; extended-message negotiation not implemented",
        ));
    }
    ...
    match b[18] {
        1 => {
            ...
            let options = usize::from(b[28]);
            if 29 + options != n {
                return Err(bad(
                    "bgp_open_options",
                    "inconsistent option extent; extended optional parameters unsupported",
                ));
            }
            let mut p = 29;
            while p < n {
...
fn validate_prefixes(b: &[u8]) -> Result<()> {
    let mut p = 0;
    while p < b.len() {
        let bits = usize::from(b[p]);
        if bits > 32 {
            return Err(bad("bgp_prefix", "IPv4 prefix longer than 32"));
        }
        p += 1 + bits.div_ceil(8);
        if p > b.len() {
            return Err(bad("bgp_prefix", "truncated IPv4 prefix"));
        }
    }
    Ok(())
}
