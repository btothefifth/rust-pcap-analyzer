...
#[test]
fn dnp_function_catalog_names_non_secure_and_auth_boundaries() {
    assert_eq!(dnp3::function_code(0x01).as_str(), "read");
    assert_eq!(
        dnp3::function_code(0x06).as_str(),
        "direct_operate_no_response"
    );
    assert_eq!(dnp3::function_code(0x1c).as_str(), "get_file_info");
    assert_eq!(dnp3::function_code(0x20).as_str(), "auth_request");
    assert_eq!(dnp3::function_code(0x83).as_str(), "auth_response");
    assert_eq!(dnp3::function_code(0xfe).as_str(), "unknown");
}
#[test]
fn dnp_link_control_exposes_primary_and_secondary_semantics() {
    let primary = dnp3::link_control(0xf3);
    assert!(primary.direction);
    assert!(primary.primary);
    assert_eq!(primary.fcb, Some(true));
    assert_eq!(primary.fcv, Some(true));
    assert_eq!(primary.dfc, None);
    assert_eq!(primary.reserved, None);
    assert_eq!(primary.function, 3);
    assert_eq!(primary.function_name, "confirmed_user_data");

    let secondary = dnp3::link_control(0x9b);
    assert!(secondary.direction);
    assert!(!secondary.primary);
    assert_eq!(secondary.fcb, None);
    assert_eq!(secondary.fcv, None);
    assert_eq!(secondary.dfc, Some(true));
    assert_eq!(secondary.reserved, Some(false));
    assert_eq!(secondary.function, 11);
    assert_eq!(secondary.function_name, "link_status");
    assert_eq!(
        dnp3::link_control(0x8f).function_name,
        "link_service_not_implemented"
    );
}
#[test]
...
fn bad_link_between_application_fragments_cannot_bridge_to_success() {
    ...
    assert!(parsed
        .issues
        .iter()
        .any(|i| i.code == "dnp3_link_boundary_lost_no_resynchronization"));
    assert!(!parsed
        .issues
        .iter()
