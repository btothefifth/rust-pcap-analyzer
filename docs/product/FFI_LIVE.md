...
The current Windows host completed the native Rust build plus MSVC link/runtime
cycle gate with the checked-in linked consumer harness. POSIX/Linux and other
target ABIs still need their own native link/run gates; a Windows pass is not an
inference that those environments match.

Windows/MSVC link/run:
...
The Windows harness link adds `ws2_32`, `userenv` and `ntdll` for Rust standard
library symbols required by this artifact. The latest local receipt records link
and execution PASS.
