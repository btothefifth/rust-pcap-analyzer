...
selection/messages/issues, transaction candidates, diagnostics and coverage cuts.
Consumers should retain events whose types they do not yet interpret. A future
incompatible contract must change the schema identifier; v1 is still a release-
review candidate, not a promised Rust ABI.
...
message/event limits and routes provenance through the sink. `TransactionCorrelator`
