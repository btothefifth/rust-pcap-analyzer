...
fragments, headers, unsolicited handling and transaction candidates. A bounded subset
of standard object bodies is decoded by the root semantic parser and reused by the
workflow projection; see [DNP3 workflow coverage](DNP3_WORKFLOWS.md) for the explicit
layouts and regression boundary. This is not exhaustive object, qualifier, selector,
or device-semantic coverage. Configured-port UDP analysis is datagram-local; it does
not reorder, retransmit, or join separate UDP datagrams. Full IEEE 1815 semantics,
secure authentication and all vendor variations are not implemented. This is not an
