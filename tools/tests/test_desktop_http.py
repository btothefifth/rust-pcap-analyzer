...
from tools.desktop.server import Manager,Server,WorkspaceStateUnavailable,read_workspace_state
...
class HttpTests(unittest.TestCase):
    ...
    def job(self):
        ...
        while time.monotonic()<deadline:
            ...
            self.assertEqual(status,200,f"status endpoint returned {data!r}")
            self.assertIsInstance(data,dict,f"status endpoint returned {data!r}")
            self.assertIn('state',data,f"status response has no state: {data!r}")
    ...
    def test_status_retries_transient_state_file_permission_error(self):
        job=self.job();calls=[]
        def transient_denial(path,limit):
            if not calls:
                calls.append(path)
                raise PermissionError('simulated Windows sharing violation')
            return read_json(path,limit)
        with patch('tools.desktop.server.read_json',side_effect=transient_denial):
            status,data=self.request('/api/status?job='+job)
        self.assertEqual(status,200,data)
        self.assertEqual(data['id'],job)
        self.assertEqual(len(calls),1)
    def test_jobs_retries_transient_state_file_permission_error(self):
        job=self.job();calls=[]
        def transient_denial(path,limit):
            if not calls:
                calls.append(path)
            ...
            return read_json(path,limit)
        with patch('tools.desktop.server.read_json',side_effect=transient_denial):
            status,data=self.request('/api/jobs')
        self.assertEqual(status,200,data)
        self.assertIn(job,[item['id']for item in data['jobs']])
        self.assertEqual(len(calls),1)
    def test_internal_state_permission_is_redacted_503_not_authorization_403(self):
        job=self.job()
        with patch('tools.desktop.server.read_workspace_state',side_effect=WorkspaceStateUnavailable(r'C:\private\state.json')):
            status,data=self.request('/api/status?job='+job)
        self.assertEqual(status,503)
        self.assertEqual(data,{'error':'workspace state temporarily unavailable'})
    def test_workspace_state_permission_retry_exhaustion_is_bounded(self):
        attempted=[]
        def denied(path,limit):
            attempted.append(path)
            raise PermissionError(r'C:\private\state.json')
        with patch('tools.desktop.server.read_json',side_effect=denied),patch('tools.desktop.server.time.sleep') as sleep:
            with self.assertRaises(WorkspaceStateUnavailable) as raised:
                read_workspace_state('unavailable-state.json',attempts=4,retry_delay=0.01)
        self.assertEqual(len(attempted),4)
        self.assertEqual(sleep.call_count,3)
        self.assertEqual(str(raised.exception),'workspace state temporarily unavailable')
