...
use crate::{
    ...
    plugin::{Context, MessageRecord, Registry},
    Error, Result,
};
use pcap_evidence::{
    ...
    fn flow_streams(
        ...
        for direction in 0..2 {
            ...
            let ctx = Context {
                ...
                ambiguous: !stream.conflicts.is_empty(),
                boundary_verified: stream.anchored_by_syn,
                modbus_ports: self.c.base.modbus_ports.clone(),
            };
            self.registry
                .stream(&ctx, stream, self.c, self.sink, messages, remaining)?;
        }
        Ok(())
    }
    fn flush(&mut self, mut a: Active, reason: &'static str) -> Result<()> {
        if let Some(tracker) = a.tracker.take() {
            for flow in tracker.finish(self.c.base.overlap_policy)? {
                ...
                self.flow_streams(id, &flow, &mut a.messages, &mut a.remaining)?;
                let mut e = Event::new(
                    EventKind::FlowEnd,
