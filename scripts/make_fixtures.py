...
# These fixtures are authored semantic vectors rather than outputs of the
# deterministic container generator below.  They still belong in the single
# fixture inventory so reference_verify.py can hash every checked-in fixture.
# The generator reads them for manifest construction but never overwrites
# their contents.
AUTHORED_FIXTURES = (
    "dnp3_confirmations.hex",
    "dnp3_confirmations.json",
    "dnp3_confirmations_capture.hex",
)
...
def authored_fixtures() -> dict[str, bytes]:
    out = {}
    for name in AUTHORED_FIXTURES:
        path = FIX / name
        if not path.is_file():
            raise SystemExit(f"missing authored fixture: {name}")
...
def main() -> int:
    ...
    files = fixtures()
    files.update(authored_fixtures())
    ...
    for name, data in files.items():
        ...
        elif name not in AUTHORED_FIXTURES:
