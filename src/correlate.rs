...
mod dnp3_confirmations;
pub use dnp3_confirmations::{
    dnp3_confirmation_candidates, Dnp3ConfirmationCandidate, Dnp3ConfirmationFragment,
    Dnp3ExcludedConfirmationTarget, FragmentRef,
};
...
pub fn transactions(
                ...
                for (mi, message) in result.messages.iter().enumerate() {
                    ...
                    let verified = if message.complete && application.direction <= 1 {
                        ...
                        Err(Error::new(
                            ErrorCode::Truncated,
                            0,
                            "dnp3_pairing_incomplete_or_direction_unknown",
                            "not pairable",
                        ))
                    };
                    if verified
                        .as_ref()
                        .is_err_and(|e| e.code == ErrorCode::LimitExceeded)
                    {
                        return Err(Error::limit("dnp3_correlation_work"));
                    }
                    let no_response = message.class == MessageClass::Request
                        && matches!(message.function, 6 | 8 | 10 | 12);
                    if !message.complete
                        || verified.is_err()
                        || no_response
                        || !matches!(
                    ...
                    {
                        let (status, reason) = if !message.complete {
                            ("incomplete", "application_fragments_missing")
                        } else if let Err(e) = verified {
                            ("unclassified", e.field)
                        } else if no_response {
                            (
                                "no_response_expected",
                                "captured_function_explicitly_suppresses_response",
                            )
                        } else {
                            match message.class {
    ...
    for ((protocol, flow, _, _, _, _), group) in groups {
        ...
        let dnp_same_direction = protocol == "dnp3"
            && group.requests.len() == 1
            && group.responses.len() == 1
            && applications[group.requests[0].application].direction
                == applications[group.responses[0].application].direction;
        let (status, reason) = match (group.requests.len(), group.responses.len()) {
            (1, 1) if dnp_same_direction => (
                "ambiguous",
