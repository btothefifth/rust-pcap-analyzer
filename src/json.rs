...
impl Json {
    ...
    /// Exact UTF-8 encoded size without constructing the serialized document.
    pub fn encoded_len_bounded(&self, limit: usize) -> crate::Result<usize> {
        self.encoded_len()
            .filter(|size| *size <= limit)
            .ok_or_else(|| crate::Error::limit("report_bytes"))
    }
    /// Check the exact size and reserve fallibly before rendering. A logical
    /// output cap does not imply a cap on the allocator's actual reservation.
    pub fn encode_bounded(&self, limit: usize) -> crate::Result<String> {
        let size = self.encoded_len_bounded(limit)?;
        let mut out = String::new();
        out.try_reserve_exact(size)
            .map_err(|_| crate::Error::limit("report_allocation"))?;
        self.render(&mut out);
        Ok(out)
    }
    /// Append atomically with respect to size/allocation failures. `limit`
    /// covers the entire destination, including the bytes already present.
    pub fn append_bounded(&self, out: &mut String, limit: usize) -> crate::Result<()> {
        let remaining = limit
            .checked_sub(out.len())
            .ok_or_else(|| crate::Error::limit("report_bytes"))?;
        let size = self.encoded_len_bounded(remaining)?;
        out.try_reserve(size)
            .map_err(|_| crate::Error::limit("report_allocation"))?;
        self.render(out);
        Ok(())
    }
    /// Reserve the document AND its newline together; no infallible capacity
    /// growth is deferred until after the bounded document has been encoded.
    pub fn encode_bounded_line(&self, limit: usize) -> crate::Result<String> {
        let size = self.encoded_len_bounded(
            limit
                .checked_sub(1)
                .ok_or_else(|| crate::Error::limit("report_bytes"))?,
        )?;
    ...
    fn encoded_len(&self) -> Option<usize> {
        match self {
            ...
            Self::Number(n) => Some(if *n == 0 { 1 } else { n.ilog10() as usize + 1 }),
