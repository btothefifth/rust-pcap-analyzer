...
pub enum Value {
    ...
    Float32Bits(u32),
    Float64Bits(u64),
    OctetsSha256 { length: usize, digest: [u8; 32] },
    Milliseconds48(u64),
...
impl Value {
    fn kind(&self) -> &'static str {
        match self {
            ...
            Self::OctetsSha256 { .. } => "octets_sha256",
    ...
    fn json(&self) -> Json {
        match self {
            ...
            Self::OctetsSha256 { length, digest } => Json::object([
                ("length", length.to_string().into()),
                ("sha256", sha256::hex(digest).into()),
            ]),
